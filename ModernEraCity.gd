...
signal ModernEraCityTexture

func _on_ModernEraCityButton_pressed():
	if get_tree().current_scene.name == "AttackersPlace":
		...
		$"/root/Autoload".ModernEraCityBool = true
	...
	if get_tree().current_scene.name == "DeffendersPlace":
		...
		$"/root/AutoloadDeffenders".ModernEraCityBool = true
		...
		emit_signal("ModernEraCityTexture")
